<template>
  <el-container class="disk-personal-page">
    ...
    <el-main class="right-content">
      <div class="file-box">
        <div class="title">
          <font-awesome-icon fas icon="list"></font-awesome-icon>&nbsp;
          文件列表&nbsp;
          <el-tag v-show="currentNode?.Name" class="tag" effect="dark">{{
            currentNode?.Name
          }}</el-tag>
        </div>
        <div class="btn-title">
          ...
          >
            <el-button :disabled="!currentNode?.Name">
              <font-awesome-icon fas icon="cloud-arrow-up"></font-awesome-icon
              >&nbsp;上传文件</el-button
            >
          </el-upload>
          <el-button
            :disabled="fileSelections.length < 1"
            @click="bulkDownloadFile"
            class="ofa-ml10"
          >
          ...
          <el-table-column label="预览" prop="Title" width="200">
            <template #default="scope">
              <el-image
                ...
                :preview-src-list="[domain + scope.row.Url]"
              ></el-image>
            </template>
          </el-table-column>
...
function findFolder(
  id: string,
  sources: IDiskFolderTree[]
): IDiskFolderTree | undefined {
...
function delTrue() {
  ElMessageBox.confirm(
    `确认要彻底删除${currentNode.value.Name}?删除后不可恢复`,
    '敏感操作提示',
    {
      type: 'warning',
...
function updateFileName(item: IDiskFile) {
  ElMessageBox.prompt('请输入要修改的新文件名，最多50个字符', '修改文件名', {
    ...
    API.updateFolderFile(
      currentNode.value.Id,
      Object.assign({ Id: item.Id, Title: value })
    ).then(res => {
      if (res.Status) {
        getFiles()
      }
...
function filesToRar(arr: IDiskFile[], filename: string) {
  ...
  for (const item of arr) {
    const promise = getImgArrayBuffer(domain.value + item.Url).then(
      (data: any) => {
