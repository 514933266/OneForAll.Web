  ...
  <div class="personal-info">
    <div class="top-box">
      <div class="header-box">
        ...
        >
          <el-avatar
            :src="domain + userStoreRef.user.value?.IconUrl"
            class="avatar-box"
          >
        ...
        <div class="section-text">
          <div class="section-text-name ofa-text-title">
            {{ userStoreRef.user.value?.Name }}({{
              userStoreRef.user.value?.UserName
            }})
            <el-button
              v-show="!isShowSignatureInput"
              link
              @click="isShowSignatureInput = true"
              type="primary"
            >
              写工作签名
            </el-button>
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-timeline class="time-box">
                <el-timeline-item
                  v-for="(item, index) in personalHistories"
                  :key="index"
                  type="primary"
                >
    ...
    <div class="info-box">
      <div class="left-box">
        <div class="col">
          <div class="ofa-mb10">
            工号：<label class="ofa-text-title">{{
              personalFile?.WorkNumber
            }}</label>
          </div>
          <div class="ofa-mb10">
            状态：<label class="ofa-text-title">{{
              personalFile?.EmployeeStatus
            }}</label>
          </div>
        </div>
        <div class="col">
          <div class="ofa-mb10">
            所属部门：<label class="ofa-text-title">{{ teamName }}</label>
          </div>
          <div class="ofa-mb10">
            入职时间：<label class="ofa-text-title">{{
              dayjs(personalFile?.EntryDate).format('YYYY-MM-DD')
            }}</label>
          </div>
        </div>
        <div class="col">
          <div class="ofa-mb10">
            职级：<label class="ofa-text-title">{{ personalFile?.Job }}</label>
          </div>
        </div>
      </div>
      <div class="right-box">
        <div class="col">
          <div class="text-top ofa-text">入职天数</div>
          <div class="text-num ofa-text-title">{{ joinDay }}</div>
        </div>
        <el-divider direction="vertical" />
        <div class="col">
          <div class="text-top ofa-text">司龄</div>
          <div class="text-num ofa-text-title">{{ joinAge }}</div>
        </div>
        <el-divider direction="vertical" />
        <div class="col">
          <div class="text-top ofa-text">工作年限</div>
          <div class="text-num ofa-text-title">{{ workAge }}</div>
        </div>
      </div>
    </div>
  </div>
...
const workAge = computed(() => {
  if (personalFile.value?.ExtendInformations) {
    return personalFile.value?.ExtendInformations.find(
      w => w.Name === 'WorkAge'
    )?.Value
  }
  return 0
...
const joinAge = computed(() => {
  if (personalFile.value?.JoinAge) {
    return personalFile.value?.JoinAge
  }
  return 0
}) // 司龄
...
const teamName = computed(() => {
  if (personalFile.value?.ExtendInformations) {
    return personalFile.value?.ExtendInformations.find(w => w.Name === 'Team')
      ?.Value
  }
  return ''
}) // 直属团队
