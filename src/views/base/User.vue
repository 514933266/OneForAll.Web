<template>
	<el-container class="user-page">
		<el-header class="header">
			<span>
				<el-input enterable v-model="searchOption.key" placeholder="用户账号 / 名称" style="width: 260px"
					class="ofa-mr10">
					<template #prefix>
		...
		<el-main class="content">
			...
			<el-table v-loading="loading" :data="list" class="ofa-table">
				<el-table-column prop="Name" label="名称" width="200">
					<template #default="scope">
						<div class="user-icon-box">
							...
							<label class="user-name">{{ scope.row.Name }}</label>
						</div>
					</template>
				</el-table-column>
				...
				<el-table-column label="账号类型" width="200">
					<template #default="scope">
						<el-tag :type="scope.row.IsDefault ? 'primary' : 'info'"> {{ scope.row.IsDefault ? '管理员' :
							'普通账号' }}</el-tag>
					</template>
				</el-table-column>
				<el-table-column label="最后登录时间" width="180">
					<template #default="scope">{{ scope.row.LastLoginTime ? scope.row.LastLoginTime : '未登录'
						}}</template>
				</el-table-column>
				...
				<el-table-column label="操作" align="center" width="200" fixed="right">
					<template #default="scope">
						...
						<el-button v-if="permission.Update" link type="warning" @click="reset(scope.row)">重置密码
						</el-button>
						<el-button v-if="permission.Delete" link type="danger" @click="del(scope.row)"> 删除</el-button>
					</template>
				</el-table-column>
			</el-table>
	...
	<el-drawer v-model="drawerVisiable" :modal="false" :show-close="false" direction="rtl" size="460"
		...
		<el-form status-icon ref="userForm" :rules="validationRule" :model="entity" class="form" label-width="80px">
			<el-alert title="操作提示" type="warning" style="margin-bottom: 20px" :closable="false"
				description="创建账号后密码默认与账号相同，请创建后立即登录修改"></el-alert>
			<div class="row">
				<el-form-item label="账号" prop="UserName">
					<el-input v-model.trim="entity.UserName" :disabled="!isAdd" placeholder="请输入用户账号"> </el-input>
				</el-form-item>
			</div>
			<div class="row">
...
import { USER } from '@/routers/base-router'
...
const list = ref<ISysTenantUser[]>([]) // 用户列表
const entity = ref<ISysTenantUser>(Object.assign({ Name: '', UserName: '', Status: 1 })) // 实体
...
function get() {
	loading.value = true
	API.getPageUser(pageIndex.value, pageSize.value, searchOption.value.key).then(res => {
...
// 显示添加抽屉
function showDrawer(item?: ISysTenantUser) {
	if (item) {
		isAdd.value = false
		entity.value = item
	} else {
...
// 重置密码
function reset(item: ISysTenantUser) {
	ElMessageBox.confirm('确认要重置选中账号密码？该操作不可恢复，请谨慎操作！', '敏感操作提示', {
		type: 'warning',
		cancelButtonText: '放弃操作'
	}).then(() => {
		API.resetUserPwd(item.Id)
	})
}

// 删除账号
function del(item: ISysTenantUser) {
	ElMessageBox.confirm('确认要删除选中账号？删除后不可恢复，请谨慎操作！', '敏感操作提示', {
		type: 'warning',
		cancelButtonText: '放弃操作'
	}).then(() => {
