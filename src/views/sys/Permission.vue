<template>
	<el-container class="perm-page">
		<el-header class="header">
			<span>
				<el-input enterable v-model="searchOption.key" placeholder="权限名称 / 代码" style="width: 260px">
					<template #prefix>
						<font-awesome-icon fas icon="user-check"></font-awesome-icon>
					</template>
				</el-input>
				<el-cascader clearable v-model="treePath" :props="{
					children: 'Children',
					value: 'Id',
					label: 'Name',
					checkStrictly: true
				}" :show-all-levels="false" :options="menus" class="ofa-ml10" placeholder="请选择所属菜单"></el-cascader>
		...
		<el-main class="content">
			...
			<el-table v-loading="loading" :data="list" class="ofa-table">
				...
				<el-table-column prop="MenuName" label="所属菜单" width="200"></el-table-column>
				<el-table-column prop="SortCode" label="排序代码" width="200"></el-table-column>
	...
	<el-drawer v-model="drawerVisiable" :modal="false" :show-close="false" direction="rtl" size="460"
		...
		<el-form status-icon ref="permForm" :rules="validationRule" :model="entity" label-width="120px">
			<el-form-item label="所属菜单" prop="MenuId" class="menu-item">
				<el-cascader v-model="treePath" :props="{
					...
					label: 'Name',
					checkStrictly: true
				}" :show-all-levels="false" :options="menus" placeholder="请选择所属菜单"></el-cascader>
			</el-form-item>
			...
			<el-form-item label="排序" prop="SortCode">
				<el-input v-model.trim="entity.SortCode" placeholder="请输入排序代码"></el-input>
			</el-form-item>
			<el-form-item label="备注" prop="Remark" class="remark-item">
				<el-input show-word-limit v-model="entity.Remark" type="textarea" maxlength="100"
					class="remark-textarea" placeholder="请输入权限备注">
				</el-input>
			</el-form-item>
		</el-form>
		<div class="footer">
...
const entity = ref<ISysPermission>(Object.assign({ Name: '', UserName: '', Status: 1 })) // 实体
...
watch(
	...
	newValue => {
		if (newValue) {
			...
			if (searchOption.value.menuId !== val) searchOption.value.menuId = newValue[newValue.length - 1] ?? GUID
		} else {
...
function get() {
	loading.value = true
	API.getPermissions(pageIndex.value, pageSize.value, searchOption.value.menuId, searchOption.value.key).then(res => {
...
function showDrawer(item?: ISysPermission) {
	if (item) {
		isAdd.value = false
		entity.value = item
		treePath.value = getTreePath(item.MenuId)
	} else {
		isAdd.value = true
...
// 查找指定id菜单
function findMenu(id: string, sources: ISysMenuTree[]): ISysMenuTree | undefined {
...
function del(item: ISysPermission) {
	ElMessageBox.confirm('确认要删除选中权限？删除后不可恢复，请谨慎操作！', '敏感操作提示', {
		type: 'warning',
