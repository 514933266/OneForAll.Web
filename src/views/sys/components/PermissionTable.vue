...
interface ISysMenuTreeModel extends ISysMenuTree {
  Checked: boolean
  Children: ISysMenuTreeModel[]
...
const list = ref<ISysMenuTreeModel[]>([]) // 数据源
...
function get() {
  loading.value = true
  API.getTreeMenus(true).then(res => {
...
// 初始化选中
function initTree(rows: ISysMenuTree[]): ISysMenuTreeModel[] {
  const models: ISysMenuTreeModel[] = []
  rows.forEach(row => {
    let children: ISysMenuTreeModel[] = []
...
// 设置选中行
function setCheckedRow(row: ISysMenuTreeModel) {
  const checked =
...
function findMenu(
  id: string,
  sources: ISysMenuTreeModel[]
): ISysMenuTreeModel | undefined {
...
// 选中行
function selectRow(checked: any, row: ISysMenuTreeModel) {
