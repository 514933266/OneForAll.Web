<template>
  <el-container class="login-log-page">
    <el-header class="header">
      <span>
        ...
        <span class="search-box">
          <el-input v-model="searchOption.exChangeName" placeholder="交换机名" style="width: 200px"
            class="ofa-mr10"></el-input>
          <el-input v-model="searchOption.queueName" placeholder="队列名" style="width: 200px" class="ofa-mr10"></el-input>
          <el-input v-model="searchOption.routeKey" placeholder="路由名" style="width: 200px" class="ofa-mr10"></el-input>
    ...
    <el-main class="content">
      ...
      <el-table v-loading="loading" :data="list" class="ofa-table">
        ...
        <el-table-column prop="ExChangeName" label="交换机" width="300"></el-table-column>
        <el-table-column prop="QueueName" label="队列名称" width="300"></el-table-column>
        <el-table-column prop="RouteKey" label="路由" width="300"></el-table-column>
        <el-table-column show-overflow-tooltip prop="OriginalMessage" label="原始消息" width="300"></el-table-column>
        <el-table-column prop="CreateTime" label="发送时间" width="200"></el-table-column>
        <el-table-column show-overflow-tooltip prop="Result" label="结果" width="100"></el-table-column>
        <el-table-column label="操作" width="120" align="center" header-align="center" fixed="right">
          <template #default="scope">
            ...
            <el-button link v-if="['Error', 'Fail'].includes(scope.row.Result)" @click="retry(scope.row)">重新发送
            </el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  <el-drawer v-model="drawerVisiable" direction="rtl" size="660" class="ofa-drawer">
    <template #header>
      <span class="title">消息明细</span>
    </template>
    <el-form status-icon ref="messageForm" :model="entity" label-width="120px">
      <el-form-item label="状态" prop="Status">
        <el-text :type="getStatusType(entity.Status)">{{ getStatusStr(entity.Status) }}</el-text>
      </el-form-item>
      <el-form-item label="交换机" prop="ExChangeName">
        {{ entity.ExChangeName }}
      </el-form-item>
      ...
      <el-form-item label="原始消息" prop="OriginalMessage">
        <el-input type="textarea" autosize
          :value="JSON.stringify(JSON.parse(entity.OriginalMessage), null, 4)"></el-input>
      </el-form-item>
    </el-form>
    <div class="footer">
      <el-button type="warning" @click="drawerVisiable = false">关闭</el-button>
    </div>
  </el-drawer>
</template>
...
const entity = ref<IUmsMessageRecord>(
  Object.assign({})
) // 实体
...
// 显示添加抽屉
function showDrawer(item: IUmsMessageRecord) {
  entity.value = item
