<template>
  <el-container class="job-level-page">
...
const list = ref<IOAJobLevel[]>([]) // 列表
const entity = ref<IOAJobLevel>(Object.assign({})) // 实体
...
const validationRule = reactive<FormRules<any>>({
  Name: [
    { required: true, message: '请先填写职级名称' },
    { min: 2, max: 20, message: '职级名称长度在2~20之间' }
  ],
...
// 获取分页列表
function get() {
  API.getJobLevels(searchOption.value.key).then(res => {
    list.value = res
  })
}
...
// 显示添加抽屉
function showDrawer(item?: IOAJobLevel) {
  if (item) {
    isAdd.value = false
    entity.value = item
  } else {
...
function submit() {
  typeFormRef.value?.validate((valid: boolean) => {
    if (valid) {
      if (isAdd.value) {
        API.addJobLevel(entity.value).then(res => {
          if (res.Status) get()
        })
      } else {
        API.updateJobLevel(entity.value)
      }
    }
  })
}
...
function del(item: IOAJobLevel) {
  ElMessageBox.confirm(
    ...
    API.delJobLevel(item.Id).then(res => {
      if (res.Status) {
        get()
      }
    })
