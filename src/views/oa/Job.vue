	...
	<el-container class="job-page">
		...
		<div class="title">
			...
			<div>
				<el-button link @click="toJobLevelPage" type="primary"
					><font-awesome-icon fas icon="cog"></font-awesome-icon>&nbsp;职级管理</el-button
				>
				<span class="right-line">|</span>
				<el-button link @click="toJobDutyPage" type="primary"
					><font-awesome-icon fas icon="bell"></font-awesome-icon>&nbsp;职务管理</el-button
				>
			</div>
		</div>
		<!-- desc -->
		<div class="title-desc">
			职位管理（共<el-text type="primary">&nbsp;{{ list.length }}&nbsp;</el-text>条数据）
		</div>
		<!-- 内容区域 -->
		<div class="job-content">
			<div class="job-left-tab">
				...
				<div class="tab-all">
					<el-button link @click="changeType(GUID)" type="primary">全部</el-button>
					<el-button link @click="showTypeDialog()"><font-awesome-icon fas icon="plus"></font-awesome-icon></el-button>
				</div>
				<el-tree
					default-expand-all
			...
			<div class="job-right-table">
				<div class="search-box">
					<div class="left">
						<el-input v-model="searchOption.key" placeholder="搜索职位名称" style="width: 200px">
							<template #prefix>
						...
						<TeamCascader
							...
							style="width: 200px"
						></TeamCascader>
						<el-select clearable v-model="searchOption.isEnabled" placeholder="是否启用" class="ofa-ml10" style="width: 200px">
							<el-option label="已启用" :value="true"></el-option>
							<el-option label="未启用" :value="false"></el-option>
						</el-select>
						<el-button link @click="filterJobs" type="primary" class="ofa-ml10">筛选</el-button>
					</div>
					<div>
				...
				<el-table :data="filterList" class="ofa-table" height="600">
					...
					<el-table-column label="操作" width="200">
						<template #default="scope">
							<el-button link @click="showDrawer(scope.row)" type="primary">编辑</el-button>
							<el-button link @click="stop(scope.row)" :type="scope.row.IsEnabled ? 'danger' : 'success'">
				...
				<el-drawer v-model="drawerVisiable" direction="rtl" class="ofa-drawer" size="380px">
					<template #header>
						<span class="title">{{ isAdd ? '添加' : '编辑' }}职位</span>
					</template>
					<el-form ref="jobFormRef" :model="entity" label-width="120px" :rules="validationRule">
						<el-form-item label="职位名称" prop="Name">
							<el-input v-model="entity.Name" placeholder="职位名称"></el-input>
						</el-form-item>
						<el-form-item label="部门" prop="TeamId">
							<TeamCascader showRoot v-model="entity.TeamId" rootName="无" placeholder="请选择所属部门"></TeamCascader>
						</el-form-item>
						<el-form-item label="职位类别" prop="TypeId">
							<el-select clearable v-model="entity.TypeId" placeholder="职位类别" class="mr10">
								<el-option label="无" :value="GUID"></el-option>
								<el-option v-for="item in types" :key="item.Id" :label="item.Name" :value="item.Id"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="对应职级" prop="LevelId">
							<el-select clearable v-model="entity.LevelId" placeholder="对应职级" class="mr10">
								<el-option label="无" :value="GUID"></el-option>
								<el-option v-for="item in levels" :key="item.Id" :label="item.Name" :value="item.Id"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="对应职务" prop="DutyId">
							<el-select clearable v-model="entity.DutyId" placeholder="对应职务" class="mr10">
								<el-option label="无" :value="GUID"></el-option>
		...
		>
			<el-form :model="typeEntity" :rules="jobTypeValidationRule" ref="typeFormRef" label-width="120px">
				<el-form-item prop="Name" label="职位分类名称">
					<el-input v-model.trim="typeEntity.Name" placeholder="请输入内容"></el-input>
				</el-form-item>
				<el-form-item prop="Remark" label="备注">
					<el-input v-model="typeEntity.Remark" :rows="4" type="textarea" placeholder="请输入备注内容"></el-input>
				</el-form-item>
			</el-form>
			<template #footer>
...
function del(item: IOAJob) {
	ElMessageBox.confirm('确认要删除选中数据？删除后不可恢复，请谨慎操作！', '敏感操作提示', {
		type: 'warning',
