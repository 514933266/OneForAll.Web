<template>
  <el-container class="person-birthday-page">
    <el-header class="header">
      <div class="title-box">
        <span class="title">员工生日 </span>
        <div class="title-total">
          即将生日员工（共<el-text type="primary"
            >&nbsp;{{ list.length }}&nbsp;</el-text
          >人）
        </div>
      </div>
    ...
    <el-main class="content">
      <el-table :data="list" class="ofa-table">
        ...
        <el-table-column label="生日" prop="Birthday" width="100">
          <template #default="scope">{{
            scope.row.Birthday
              ? dayjs(scope.row.Birthday).format('MM月DD日')
              : ''
          }}</template>
        </el-table-column>
      </el-table>
...
const list = ref<IOAPersonBirthday[]>([]) // 用户列表
// 搜索条件
...
function get() {
  ...
  API.getBirthPersons(searchOption.value.teamId, startDate, endDate).then(
    res => {
      list.value = res
    }
  )
