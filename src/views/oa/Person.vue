<template>
	<el-container class="person-page">
		<div class="header">
			<el-col class="count-box">
				<div class="count-item">
					<font-awesome-icon fas icon="users" class="icon icon-primary"></font-awesome-icon>
					<div class="r-label">
			...
			<el-col class="count-box">
				<div class="count-item">
					<font-awesome-icon fas icon="user-tie" class="icon icon-warning"></font-awesome-icon>
					<div class="r-label">
			...
			<el-col class="count-box">
				<div class="count-item">
					<font-awesome-icon fas icon="user-clock" class="icon icon-success"></font-awesome-icon>
					<div class="r-label">
			...
			<el-col class="count-box">
				<div class="count-item">
					<font-awesome-icon fas icon="user-graduate" class="icon icon-danger"></font-awesome-icon>
					<div class="r-label">
		...
		<div class="w-p">
			...
			<el-divider direction="vertical"></el-divider>
			<el-upload :action="API.getImportPersonExcelUrl()" :show-file-list="false">
		...
		<div class="w-p-2">
			...
			<span class="search-box">
				...
				<el-input v-model="searchOption.key" class="search-input" placeholder="请输入名称搜索"> </el-input>
		...
		<el-table v-loading="loading" :data="list" class="ofa-table">
			<el-table-column width="50" align="center" fixed>
				<template #header>
					<el-tooltip class="item" effect="dark" content="设置表头" placement="top">
						<font-awesome-icon @click="settingDrawerVisiable = true" fas icon="cog" style="cursor: pointer"> </font-awesome-icon>
					</el-tooltip>
				</template>
				<template #default="scope">{{ scope.$index + 1 + (pageIndex - 1) * pageSize }}</template>
			</el-table-column>
			<el-table-column v-for="item in checkedFields" :key="item.Id" :prop="item.Name" :label="item.Text" min-width="200">
				<template #default="scope">
					...
					<label v-else-if="item.Type === 3">{{
						scope.row[item.Name] ? dayjs(scope.row[item.Name]).format('YYYY年MM月DD日') : ''
					}}</label>
			...
			<el-table-column label="操作" width="160" fixed="right" align="center">
				<template #default="scope">
					<el-button link type="primary" @click="toPersonDetailPage(scope.row)">修改 </el-button>
	...
	<!-- 人员抽屉 -->
	<el-drawer v-model="drawerVisiable" :modal="false" :show-close="false" direction="rtl" size="380px" class="ofa-drawer">
		<template #header>
			<span class="title">创建人员档案</span>
		</template>
		<el-form status-icon ref="personFormRef" :rules="validationRule" :model="entity" class="form" label-width="80px">
			...
			<el-form-item label="入职时间" prop="EntryDate">
				<el-date-picker v-model="entity.EntryDate" value-format="YYYY-MM-DD" type="date" placeholder="入职时间"> </el-date-picker>
			</el-form-item>
			<el-form-item label="员工类型" prop="EmployeeType">
				<el-select v-model="entity.EmployeeType" placeholder="员工类型">
					<el-option v-for="item in PERSON_EMPLOYEE_TYPE" :key="item.value" :label="item.label" :value="item.value"> </el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="员工状态" prop="EmployeeStatus">
				<el-select v-model="entity.EmployeeStatus" placeholder="员工状态">
					<el-option v-for="item in PERSON_EMPLOYEE_STATUS" :key="item.value" :label="item.label" :value="item.value">
					</el-option>
				</el-select>
			</el-form-item>
	...
	<!-- 字段抽屉 -->
	<el-drawer v-model="settingDrawerVisiable" :modal="false" :show-close="false" direction="rtl" size="380px" class="ofa-drawer">
		<template #header>
			<span class="title">自定义表头</span>
		</template>
		<div class="field-pannel">
			<div class="title">已选择字段<el-text type="info" class="ofa-ml4">可拖拽标签调整顺序</el-text></div>
			<Draggable v-model="checkedFields" :animation="100" item-key="Id" class="field-checked-box">
		...
		<div class="footer">
			...
			<el-button type="warning" @click="settingDrawerVisiable = false"> 取消 </el-button>
		</div>
	</el-drawer>
	<!-- 导出人员档案弹窗 -->
	<el-dialog custom-class="ofa-dialog" title="导出人员资料" v-model="exportExcelDialogVisiable" width="800">
		<el-alert :closable="false" title="请选择要导出的人员信息字段，如不选择将会按照默认信息导出" type="warning"> </el-alert>
		<el-form :model="exportParameter" label-width="120px" class="export-form" style="margin-top: 10px">
			<el-form-item label="员工类型" prop="EmployeeType">
				<el-select v-model="exportParameter.EmployeeType" placeholder="员工类型" style="width: 600px">
					<el-option value="" label="全部"></el-option>
					<el-option v-for="item in PERSON_EMPLOYEE_TYPE" :key="item.value" :label="item.label" :value="item.label"> </el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="员工状态" prop="EmployeeStatus">
				<el-select v-model="exportParameter.EmployeeStatus" placeholder="员工状态" style="width: 600px">
					<el-option value="" label="全部"></el-option>
					<el-option v-for="item in PERSON_EMPLOYEE_STATUS" :key="item.value" :label="item.label" :value="item.label">
					</el-option>
				</el-select>
			</el-form-item>
			...
			<el-form-item label="职级" prop="Job">
				<el-select multiple filterable v-model="exportParameter.Jobs" style="width: 600px">
					<el-option v-for="item in jobs" :key="item.Id" :value="item.Name" :label="item.Name" placeholder="岗位职级"></el-option>
				</el-select>
			</el-form-item>
...
function getFieldSettings(columnSetting: IOATableColumnSetting) {
	const defaults = ['Name', 'Job', 'MobilePhone', 'Email', 'EmployeeStatus', 'EntryDate']
	API.getFieldSettings().then(res => {
		res.forEach(e => {
			...
			e.Fields.forEach(e2 => {
				...
				if (e2.Type === 7) return
				const checkIndex = columnSetting.VisiableFields.findIndex(w => w === e2.Name)
...
function get() {
	loading.value = true
	API.getPagePerson(pageIndex.value, pageSize.value, searchOption.value.key, searchOption.value.onJobStatus).then(res => {
...
function del(item: IOAPerson) {
	ElMessageBox.confirm('确认要删除选中人员档案？删除后不可恢复，请谨慎操作！', '敏感操作提示', {
		type: 'warning',
...
function updateTableColumnSetting() {
	...
	API.updateTableColumnSetting(Object.assign({}, tableColumnEntity.value, { VisiableFields: fields }))
}
