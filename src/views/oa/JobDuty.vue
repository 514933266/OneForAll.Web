<template>
  <el-container class="job-duty-page">
...
const list = ref<IOAJobDuty[]>([]) // 列表
const entity = ref<IOAJobDuty>(Object.assign({})) // 实体
...
const validationRule = reactive<FormRules<any>>({
  Name: [
    { required: true, message: '请先填写职务名称' },
    { min: 2, max: 20, message: '职务名称长度在2~20之间' }
  ],
...
// 获取分页列表
function get() {
  API.getJobDutys(searchOption.value.key).then(res => {
    list.value = res
  })
}
...
// 显示添加抽屉
function showDrawer(item?: IOAJobDuty) {
  if (item) {
    isAdd.value = false
    entity.value = item
  } else {
...
function submit() {
  typeFormRef.value?.validate((valid: boolean) => {
    if (valid) {
      if (isAdd.value) {
        API.addJobDuty(entity.value).then(res => {
          if (res.Status) get()
        })
      } else {
        API.updateJobDuty(entity.value)
      }
    }
  })
}
...
function del(item: IOAJobDuty) {
  ElMessageBox.confirm(
    ...
    API.delJobDuty(item.Id).then(res => {
      if (res.Status) {
        get()
      }
    })
